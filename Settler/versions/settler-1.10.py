...
class InstallDialog(Adw.Dialog):
    def __init__(self, commands, title="Instalacja", parent=None):
        ...
        # Sprawdź czy komendy wymagają sudo (pkexec / sudo / paru / yay)
        needs_sudo = any(cmd_needs_privilege(cmd) for _, cmd in commands)
        if needs_sudo:
    ...
    def _ask_password(self):
        pwd_dialog = AskPasswordDialog(
            self._start_with_password,
            on_cancel=self.close,
        )
        pwd_dialog.present(self._parent or self)
    ...
    def _run(self, password):
        # Tymczasowy askpass — sudo/paru mogą go użyć bez TTY
        askpass_path = None
        env = os.environ.copy()
        if password:
        ...
        try:
            ...
            for i, (label, cmd) in enumerate(self.commands):
                ...
                if password and isinstance(cmd, (list, tuple)) and 'pkexec' in cmd:
