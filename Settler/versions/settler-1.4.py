...
GLOBAL_CSS = """
/* Solid background to prevent wallpaper bleed.
   We target our classes + known Adwaita nodes. No universal * to avoid parser errors.
*/
window,
window.background,
.background,
...
class AppearancePage(Adw.PreferencesPage):
    def __init__(self, win):
        ...
        win_group.set_description('Zapisywane do ~/.config/gtk-4.0/gtk.css — działają w GTK4')
    ...
    def _apply_dynamic_css(self):
        ...
        if 'window_bg_color' in v:
            c = v['window_bg_color']
            # Force on all structural nodes that can paint the window bg (very aggressive)
            rules.append(
                ...
                f'headerbar, .settler-root, .settler-bg, '
                f'.settler-root > *, .settler-bg > * '
                f'{{ background-color: {c}; background-image: none !important; }}'
            )
        if 'window_fg_color' in v:
            c = v['window_fg_color']
            rules.append(f'.settler-root label {{ color: {c}; }}')
        if 'headerbar_bg_color' in v:
            c = v['headerbar_bg_color']
            rules.append(f'headerbar, .settler-root headerbar {{ background-color: {c}; }}')
        if 'sidebar_bg_color' in v:
            c = v['sidebar_bg_color']
            rules.append(f'.navigation-sidebar, .settler-root .navigation-sidebar {{ background-color: {c}; }}')
        if 'card_bg_color' in v:
            c = v['card_bg_color']
            rules.append(f'row, .settler-root row {{ background-color: {c}; }}')
        if 'accent_color' in v:
            c = v['accent_color']
            rules.append(f'.settler-root button.suggested-action {{ background-color: {c}; }}')
            rules.append(f'.settler-root progressbar > trough > progress {{ background-color: {c}; }}')
            rules.append(f'.settler-root switch:checked {{ background-color: {c}; }}')
            rules.append(f'.settler-root selection {{ background-color: {c}; }}')
...
class SettlerWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        # Strong solid background provider using theme color (supports light/dark)
...
class SettlerApp(Adw.Application):
    ...
    def do_activate(self):
        ...
        # Extra early force using theme color (light/dark aware)
