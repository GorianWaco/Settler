...
ACCENT_COLORS = [
    ...
    # Dodatkowe (GTK4 CSS)
    {"name": "Granat",         "css": "acc-navy",        "value": "",       "hex": "#1a3a5c", "official": False},
    {"name": "Cyjan",          "css": "acc-cyan",        "value": "",       "hex": "#00acc1", "official": False},
    {"name": "Limonka",        "css": "acc-lime",        "value": "",       "hex": "#7cb342", "official": False},
    {"name": "Magenta",        "css": "acc-magenta2",    "value": "",       "hex": "#d81b60", "official": False},
    {"name": "Złoty",          "css": "acc-gold",        "value": "",       "hex": "#f9a825", "official": False},
    {"name": "Brązowy",        "css": "acc-brown",       "value": "",       "hex": "#6d4c41", "official": False},
    {"name": "Indygo",         "css": "acc-indigo",      "value": "",       "hex": "#3949ab", "official": False},
    {"name": "Morski",         "css": "acc-sea",         "value": "",       "hex": "#00897b", "official": False},
    {"name": "Głęboki fiol.",  "css": "acc-deeppurple",  "value": "",       "hex": "#6b21a8", "official": False},
]
...
class AppearancePage(Adw.PreferencesPage):
    def __init__(self, win):
        ...
        # Inicjalizuj CSS vars od razu
        self._dynamic_provider = None
        self._css_vars = {}
        self._load_css_vars_from_file()
        ...
        for ac in ACCENT_COLORS:
            ...
            btn.set_tooltip_text(ac['name'] + ('' if ac['official'] else ' (GTK4)'))
        ...
        # Przycisk własnego koloru
        vbox_custom = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        custom_btn = Gtk.Button()
        custom_btn.add_css_class('color-btn')
        custom_btn.set_icon_name('list-add-symbolic')
        custom_btn.set_tooltip_text('Własny kolor akcentu')
        custom_btn.connect('clicked', self._pick_custom_accent)
        lbl_c = Gtk.Label(label='Własny')
        lbl_c.add_css_class('caption')
        vbox_custom.append(custom_btn)
        vbox_custom.append(lbl_c)
        flow.append(vbox_custom)

        accent_group.add(flow)
        self.add(accent_group)

        # ─── Kolory okien (GTK4) ──────────────
        win_group = Adw.PreferencesGroup()
        win_group.set_title('Kolory okien (GTK4)')
        win_group.set_description(
            'Zapisywane do ~/.config/gtk-4.0/gtk.css ( @define-color )\n'
            'Wpływa na WSZYSTKIE aplikacje GTK4 po restarcie.\n'
            'Natychmiastowa podgląd tylko w tym programie.\n'
            'Zrestartuj inne aplikacje lub wyloguj się, aby zobaczyć zmiany w całym systemie.'
        )

        win_colors = [
            ('Tło okna',     'window_bg_color',    '#242424', 'Kolor tła okien aplikacji'),
            ('Kolor tekstu',  'window_fg_color',    '#ffffff', 'Kolor tekstu w oknach'),
            ('Nagłówek',     'headerbar_bg_color', '#303030', 'Kolor paska tytułu'),
            ('Pasek boczny',  'sidebar_bg_color',   '#2a2a2a', 'Kolor panelu bocznego'),
            ('Karta/widok',   'card_bg_color',      '#383838', 'Kolor kart i list'),
        ]
        for label, var, default, tooltip in win_colors:
            row = Adw.ActionRow()
            row.set_title(label)
            row.set_subtitle(tooltip)
            # Używamy Gtk.ColorDialogButton (nowoczesne API GTK4)
            cd = Gtk.ColorDialog()
            cd.set_with_alpha(False)
            cd.set_title(f'Wybierz: {label}')
            color_btn = Gtk.ColorDialogButton(dialog=cd)
            color_btn.set_valign(Gtk.Align.CENTER)
            color_btn.set_tooltip_text(f'@define-color {var}')
            current_val = self._read_gtk4_css_var(var) or default
            rgba = Gdk.RGBA()
            rgba.parse(current_val)
            color_btn.set_rgba(rgba)
            color_btn.connect('notify::rgba', self._on_win_color, var)
            row.add_suffix(color_btn)
            win_group.add(row)

        # Quick presets for main window background (easy way to pick gray/blue etc.)
        presets_row = Adw.ActionRow()
        presets_row.set_title("Szybkie szare/niebieskie tła")
        presets_row.set_subtitle("Zastosuj jeden z gotowych kolorów tła")
        ...
        presets = [
            ...
            ("Średni szary", "#2a2a2a"),
            ("Jasny szary", "#3a3a3a"),
            ("Ciemny niebieski", "#1a2633"),
            ("Niebieskawy", "#1e3a5f"),
            ("Systemowy", None),  # will reset to theme
        ]

        for name, hexval in presets:
            b = Gtk.Button(label=name)
            b.set_valign(Gtk.Align.CENTER)
            if hexval:
                b.connect('clicked', lambda _btn, h=hexval: self._update_css({'window_bg_color': h}))
            else:
                b.connect('clicked', lambda _btn: self._reset_window_bg_only())
            presets_box.append(b)

        presets_row.add_suffix(presets_box)
        win_group.add(presets_row)

        reset_row = Adw.ActionRow()
        reset_row.set_title('Resetuj do domyślnych')
        reset_row.set_subtitle('UsuŃ plik gtk-4.0/gtk.css')
        reset_btn = Gtk.Button(label='Reset')
        reset_btn.add_css_class('destructive-action')
        reset_btn.set_valign(Gtk.Align.CENTER)
        reset_btn.connect('clicked', self._reset_gtk4_css)
        reset_row.add_suffix(reset_btn)
        win_group.add(reset_row)
        self.add(win_group)
        ...
        # ─── Przestarzałe programy (GTK3) ──────────────
        legacy_group = Adw.PreferencesGroup()
        legacy_group.set_title("Przestarzałe programy (GTK3)")
        legacy_group.set_description("Motyw dla starszych aplikacji GTK3 (nie używających libadwaita)")

        self.legacy_combo = Adw.ComboRow()
        self.legacy_combo.set_title("Motyw GTK3")
        legacy_model = Gtk.StringList()
        for t in GTK_THEMES:
            legacy_model.append(t['name'])
        self.legacy_combo.set_model(legacy_model)
        current_gtk = gsettings_get('org.gnome.desktop.interface', 'gtk-theme')
        for i, t in enumerate(GTK_THEMES):
            if t['gtk'] == current_gtk:
                self.legacy_combo.set_selected(i)
                break

        legacy_btn = Gtk.Button(label="Zastosuj")
        legacy_btn.add_css_class('suggested-action')
        legacy_btn.set_valign(Gtk.Align.CENTER)
        legacy_btn.connect('clicked', self._apply_legacy)
        self.legacy_combo.add_suffix(legacy_btn)
        legacy_group.add(self.legacy_combo)
        self.add(legacy_group)
    ...
    # ─── Zarządzanie dynamicznym CSS ─────────────────

    def _load_css_vars_from_file(self):
        """Wczytuje zmienne z ~/.config/gtk-4.0/gtk.css — tylko poprawne hex."""
        ...
        with open(css_path) as f:
            for line in f:
                ...
                if m:
                    self._css_vars[m.group(1)] = m.group(2)
        # Przepisz plik — usuń błędne/stare wartości które GTK4 by wczytywał
        self._save_gtk4_css()

    def _init_dynamic_css(self):
        """Aplikuje zapisany stan CSS do bieżącej sesji."""
        if self._css_vars:
            self._apply_dynamic_css()

    def _read_gtk4_css_var(self, var: str) -> str:
        return self._css_vars.get(var, '')

    def _update_css(self, variables: dict):
        """Aktualizuje zmienne CSS, aplikuje natychmiast i zapisuje do pliku."""
        self._css_vars.update(variables)
        self._apply_dynamic_css()
        self._save_gtk4_css()

    def _apply_dynamic_css(self):
        """Buduje CSS z aktualnych zmiennych i aplikuje do bieżącej sesji."""
        # Ustalamy reguły CSS na podstawie zmiennych
        rules = []
        v = self._css_vars

        if 'window_bg_color' in v:
            c = v['window_bg_color']
            # Global rules so it affects the whole system (other GTK4 apps via provider + @define)
            rules.append(
                f'window, window.background, .background, '
                f'adw-application-window, adw-application-window.background, '
                f'adw-overlay-split-view, adw-toolbar-view, overlay-split-view, toolbar-view, '
                f'preferencespage, adw-preferences-page, clamp, adw-clamp, scrolledwindow, viewport, '
                f'headerbar, box, grid '
                f'{{ background-color: {c}; background-image: none !important; }}'
            )
            # Also for this app's root for extra safety
            rules.append(f'.settler-root, .settler-bg {{ background-color: {c}; background-image: none !important; }}')
        if 'window_fg_color' in v:
            c = v['window_fg_color']
            rules.append(f'label, .settler-root label {{ color: {c}; }}')
        if 'headerbar_bg_color' in v:
            c = v['headerbar_bg_color']
            rules.append(f'headerbar {{ background-color: {c}; }}')
            rules.append(f'.settler-root headerbar {{ background-color: {c}; }}')
        if 'sidebar_bg_color' in v:
            c = v['sidebar_bg_color']
            rules.append(f'.navigation-sidebar {{ background-color: {c}; }}')
            rules.append(f'.settler-root .navigation-sidebar {{ background-color: {c}; }}')
        if 'card_bg_color' in v:
            c = v['card_bg_color']
            rules.append(f'row {{ background-color: {c}; }}')
            rules.append(f'.settler-root row {{ background-color: {c}; }}')
        if 'accent_color' in v:
            c = v['accent_color']
            rules.append(f'button.suggested-action {{ background-color: {c}; }}')
            rules.append(f'.settler-root button.suggested-action {{ background-color: {c}; }}')
            rules.append(f'progressbar > trough > progress {{ background-color: {c}; }}')
            rules.append(f'switch:checked {{ background-color: {c}; }}')
            rules.append(f'selection {{ background-color: {c}; }}')
        ...
        if self._dynamic_provider:
            try:
                Gtk.StyleContext.remove_provider_for_display(
                    Gdk.Display.get_default(), self._dynamic_provider)
            except Exception:
                pass

        self._dynamic_provider = Gtk.CssProvider()
        self._dynamic_provider.load_from_string(css)
        Gtk.StyleContext.add_provider_for_display(
            Gdk.Display.get_default(),
            self._dynamic_provider,
            3000  # higher than the hard gray providers so custom colors win
        )

    def _save_gtk4_css(self):
        """Zapisuje aktualne zmienne do ~/.config/gtk-4.0/gtk.css"""
        css_path = os.path.expanduser('~/.config/gtk-4.0/gtk.css')
        os.makedirs(os.path.dirname(css_path), exist_ok=True)
        with open(css_path, 'w') as f:
            for var, val in self._css_vars.items():
                f.write(f'@define-color {var} {val};\n')

    def _set_accent(self, btn, ac):
        if ac['official']:
            gsettings_set('org.gnome.desktop.interface', 'accent-color', ac['value'])
        else:
            self._update_css({'accent_color': ac['hex'], 'accent_bg_color': ac['hex']})

    def _pick_custom_accent(self, btn):
        dialog = Gtk.ColorDialog()
        dialog.set_title('Wybierz kolor akcentu')
        dialog.set_with_alpha(False)
        initial = Gdk.RGBA()
        initial.parse(self._css_vars.get('accent_color', '#3584e4'))
        dialog.choose_rgba(self.win, initial, None, self._on_custom_accent)

    def _on_custom_accent(self, dialog, result):
        try:
            color = dialog.choose_rgba_finish(result)
            if color:
                h = '#{:02x}{:02x}{:02x}'.format(
                    int(color.red * 255), int(color.green * 255), int(color.blue * 255))
                self._update_css({'accent_color': h, 'accent_bg_color': h})
        except Exception:
            pass

    def _on_win_color(self, btn, _pspec, var):
        c = btn.get_rgba()
        h = '#{:02x}{:02x}{:02x}'.format(
            int(c.red * 255), int(c.green * 255), int(c.blue * 255))
        self._update_css({var: h})

    def _reset_gtk4_css(self, _):
        self._css_vars.clear()
        css_path = os.path.expanduser('~/.config/gtk-4.0/gtk.css')
    ...
    def _reset_window_bg_only(self):
        """Reset only the main window background color (keep other custom colors)."""
        self._css_vars.pop('window_bg_color', None)
        # Re-save without it
        css_path = os.path.expanduser('~/.config/gtk-4.0/gtk.css')
        os.makedirs(os.path.dirname(css_path), exist_ok=True)
        with open(css_path, 'w') as f:
            for var, val in self._css_vars.items():
                f.write(f'@define-color {var} {val};\n')
        # Re-apply current state (will fall back to @window_bg_color from theme)
        self._apply_dynamic_css()
    ...
    def _apply_legacy(self, _):
        """Zastosuj motyw GTK3 (Przestarzałe programy)."""
        t = GTK_THEMES[self.legacy_combo.get_selected()]
        cmds = []
        if t['pkg'] and not is_pkg_installed(t['pkg']):
            cmds.append((f"Instalacja {t['pkg']}", ['paru', '-S', '--noconfirm', t['pkg']]))
        cmds.append(("Motyw GTK3 (przestarzałe programy)", ['gsettings', 'set',
                     'org.gnome.desktop.interface', 'gtk-theme', t['gtk']]))
        InstallDialog(cmds, 'GTK3 — Przestarzałe programy', parent=self.win).present(self.win)
    ...
    def _apply_shell(self, _):
        ...
        # Ustawienie motywu Shell (schema powinno już być dostępne)
        ...
        cmds.append(("Informacja", ['echo', 'Jeśli motyw się nie zastosował — zrestartuj GNOME Shell (Alt+F2, wpisz r, Enter)']))
