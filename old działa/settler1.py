...
GLOBAL_CSS = """
window { background-color: @window_bg_color !important; }
...
ACCENT_COLORS = [
    {"name": "Niebieski",    "css_class": "acc-blue",   "value": "blue"},
    {"name": "Turkusowy",    "css_class": "acc-teal",   "value": "teal"},
    {"name": "Zielony",      "css_class": "acc-green",  "value": "green"},
    {"name": "Żółty",        "css_class": "acc-yellow", "value": "yellow"},
    {"name": "Pomarańczowy", "css_class": "acc-orange", "value": "orange"},
    {"name": "Czerwony",     "css_class": "acc-red",    "value": "red"},
    {"name": "Różowy",       "css_class": "acc-pink",   "value": "pink"},
    {"name": "Fioletowy",    "css_class": "acc-purple", "value": "purple"},
    {"name": "Szary",        "css_class": "acc-slate",  "value": "slate"},
]
...
class AppearancePage(Adw.PreferencesPage):
    def __init__(self, win):
        ...
        accent_group.set_title("Kolor wyróżnienia (libadwaita)")
        ...
        for ac in ACCENT_COLORS:
            ...
            btn.add_css_class(ac['css_class'])
            btn.set_tooltip_text(ac['name'])
            btn.connect('clicked', self._set_accent, ac['value'])
        ...
        self.cursor_size_scale.set_size_request(160, -1)
        ...
        gtk_group.set_title("Motyw GTK i Shell")
        gtk_group.set_description("Wymaga User Themes extension dla Shell")

        self.gtk_combo = Adw.ComboRow()
        self.gtk_combo.set_title("Motyw")
        gtk_model = Gtk.StringList()
        for t in GTK_THEMES:
            gtk_model.append(t['name'])
        self.gtk_combo.set_model(gtk_model)

        gtk_btn = Gtk.Button(label="Zastosuj")
        gtk_btn.add_css_class('suggested-action')
        gtk_btn.set_valign(Gtk.Align.CENTER)
        gtk_btn.connect('clicked', self._apply_gtk)
        self.gtk_combo.add_suffix(gtk_btn)
        gtk_group.add(self.gtk_combo)
        self.add(gtk_group)
    ...
    def _set_accent(self, btn, value):
        gsettings_set('org.gnome.desktop.interface', 'accent-color', value)
    ...
    def _apply_gtk(self, _):
        t = GTK_THEMES[self.gtk_combo.get_selected()]
        cmds = []
        if t['pkg'] and not is_pkg_installed(t['pkg']):
            cmds.append((f"Instalacja {t['pkg']}", ['paru', '-S', '--noconfirm', t['pkg']]))
        cmds.append(("Motyw GTK", ['gsettings', 'set',
                     'org.gnome.desktop.interface', 'gtk-theme', t['gtk']]))
        if t['shell']:
            cmds.append(("Motyw Shell", ['gsettings', 'set',
                         'org.gnome.shell.extensions.user-theme', 'name', t['shell']]))
        InstallDialog(cmds, 'Motyw GTK/Shell', parent=self.win).present(self.win)
