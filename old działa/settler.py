#!/usr/bin/env python3
"""Settler 1.0 — CachyOS Post-Install Setup GUI"""
...
class AppearancePage(Adw.PreferencesPage):
    def __init__(self, win):
        ...
        # Wczytaj aktualny motyw Shell (z timeoutem — gsettings może wisieć)
        try:
            ...
            if os.path.exists(schema_dir):
                r = subprocess.run(
                    ['bash', '-c',
                     f'GSETTINGS_SCHEMA_DIR={schema_dir} '
                     'gsettings get org.gnome.shell.extensions.user-theme name'],
                    capture_output=True, text=True, timeout=2)
                current_shell = r.stdout.strip().strip("'")
                for i, t in enumerate(GTK_THEMES):
    ...
    def _apply_dynamic_css(self):
        ...
        if 'window_bg_color' in v:
            c = v['window_bg_color']
            rules.append(f'.settler-root {{ background-color: {c}; }}')
        if 'window_fg_color' in v:
            c = v['window_fg_color']
            rules.append(f'label {{ color: {c}; }}')
    ...
    def _apply_shell(self, _):
        ...
        if idx == 0:
            # Domyślny — wyczyść motyw Shell
            shell_name = ''
        else:
            # Filtruj tylko motywy z shell (jak w modelu)
            ...
            if t['pkg'] and not is_pkg_installed(t['pkg']):
                InstallDialog(
                    [(f"Instalacja {t['pkg']}", ['paru', '-S', '--noconfirm', t['pkg']])],
                    'Instalacja motywu', parent=self.win
                ).present(self.win)
                return
        ...
        if not os.path.exists(schema_dir):
            # Sprawdź czy zainstalowane systemowo
        ...
        if schema_dir:
            cmd = f'GSETTINGS_SCHEMA_DIR={schema_dir} gsettings set '\
                  f'org.gnome.shell.extensions.user-theme name "{shell_name}"'
        else:
            cmd = f'gsettings set org.gnome.shell.extensions.user-theme name "{shell_name}"'

        cmds = [("Motyw GNOME Shell", ['bash', '-c', cmd])]
    ...
    def _add_thumb(self, path):
        try:
            ...
            # GTK4.12+: używamy MemoryTexture zamiast deprecated new_for_pixbuf
            try:
                raw = pb.get_pixels()
                ...
                tex = Gdk.MemoryTexture.new(
                    ...
                    GLib.Bytes.new(raw), pb.get_rowstride())
            except AttributeError:
                tex = Gdk.Texture.new_for_pixbuf(pb)  # fallback starsze GTK4
...
class KeydPage(Adw.PreferencesPage):
    ...
    def _enable_razer_daemon(self, _):
        user = os.environ.get('USER', 'gorian')
        cmds = [
...
class SettlerWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        sb_title = Gtk.Label(label="Settler 1.0")
